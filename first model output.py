...
def predict_game(home_team, away_team, home_odds, away_odds):
    ...
    prediction_prob = model.predict_proba(X_next_game_scaled)[0][prediction]
    ...
    home_value_bet = "Yes" if implied_home_prob < prediction_prob else "No"
    away_value_bet = "Yes" if implied_away_prob < (1 - prediction_prob) else "No"

    # Display the result
    result_label.config(text=f"Prediction: {'Home Team Wins' if prediction == 1 else 'Away Team Wins'}\n"
                             f"Confidence: {prediction_prob * 100:.2f}%\n"
